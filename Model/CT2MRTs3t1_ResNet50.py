...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_CT = np.load(image_path_CT)
        img_CT = Image.fromarray(img_CT.astype('uint8')).convert('RGB')
        img_CT= np.array(img_CT)

        #img_MRI = Image.open(image_path_MRI).convert("RGB")
        img_MRI = np.load(image_path_MRI)
        img_MRI = Image.fromarray(img_MRI.astype('uint8')).convert('RGB')
...
class MyDatasetTest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_CT, img_MRI, label, patientID
...
class CustomModel(nn.Module):
    ...
    def forward(self, image_CT, image_MRI):
        ...
        x_ct = self.resnet50(image_CT)
        x_mri = self.resnet50(image_MRI)

        # Concatenate the ResNet-50 output and extended features
        x = torch.cat([x_ct, x_mri], dim=-1)
...
def train_epoch(net, trainloader, criterion, optimizer):
    ...
    for i, data in enumerate(trainloader):
        img_ct,img_mri, labels = data

        img_ct,img_mri, labels = img_ct.to(device),img_mri.to(device), labels.to(device)

        optimizer.zero_grad()
        outputs = net(img_ct, img_mri)
...
def valid_epoch(net, valid_loader, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(valid_loader, 0):
            val_ct, val_mri, val_labels, patientsID = data

            val_ct, val_mri, val_labels = val_ct.to(device),val_mri.to(device),val_labels.to(device)

            # No need to convert labels to torch.tensor
            outputs = net(val_ct,val_mri)
...
def test_epoch(net, valid_loader, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(valid_loader, 0):
            test_ct,test_mri, test_labels, patientsID= data

            test_ct, test_mri, test_labels = test_ct.to(device), test_mri.to(device),test_labels.to(device)


            # No need to convert labels to torch.tensor
            outputs = net(test_ct,test_mri)
...
if __name__ == '__main__':
    ...
    model_save_dir = "/root/code/thesis/codeFolder/LatestDataInUse/results/models/Resnet/CT2+t2_hr_numpy_Resnet/Image"
    os.makedirs(model_save_dir, exist_ok=True)  # Ensure directory exists



    results={}
    curve_save_dir = "/root/code/thesis/codeFolder/LatestDataInUse/results/curves/Resnet/CT2+t2_hr_numpy_Resnet/Image"
    ...
    for lr in learning_rates:
        ...
        survival_results_df.to_csv(f'/root/code/thesis/codeFolder/LatestDataInUse/results/survival/ResNet50/CT2+t2_hr_numpy_Resnet/Image/survival_results_lr_{best_learning_rate}.csv', index=False)
        # Save classification report to file
        with open(f'/root/code/thesis/codeFolder/LatestDataInUse/results/reports/Resnet/CT2+t2_hr_numpy_Resnet/Image/classification_report_lr_{best_learning_rate}.txt', 'w') as f:
    ...
    result_file = "/root/code/thesis/codeFolder/LatestDataInUse/results/CT2+t2_hr_numpy_Resnet.txt"
