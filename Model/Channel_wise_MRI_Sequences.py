...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_e_thrive,img_s3t1_wats_neu_Augmented, img_t2_hr_spir_range_Augmented, label
...
class MyDatasetTest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_e_thrive,img_s3t1_wats_neu_Augmented,img_t2_hr_spir_range_Augmented, label, patientsID
...
def train_epoch(net, trainloader, criterion, optimizer):
    ...
    for i, data in enumerate(trainloader):
        img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, labels = data

        img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, labels = img_e_thrive.to(device), img_s3t1_wats_neu.to(device), img_t2_hr_spir_range.to(device), labels.to(device)

        optimizer.zero_grad()
        outputs = net(img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range)
...
def valid_epoch(net, valid_loader, criterion):
    ...
    val_f1_scores = []  
    val_predictions_all = []
    val_labels_all = []  # To store F1 scores


    with torch.no_grad():
        for i, data in enumerate(valid_loader, 0):

            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, val_labels, patientsID = data

            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, val_labels = img_e_thrive.to(device), img_s3t1_wats_neu.to(device), img_t2_hr_spir_range.to(device), val_labels.to(device)
...
def test_epoch(net, valid_loader, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(valid_loader, 0):
            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, test_labels, patientsID = data

            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, test_labels = img_e_thrive.to(device), img_s3t1_wats_neu.to(device), img_t2_hr_spir_range.to(device), test_labels.to(device)
...
if __name__ == '__main__':
    ...
    model_save_dir = "/root/code/thesis/codeFolder/LatestDataInUse/results/models/Resnet/MRICombineChannelWise/Image"
    os.makedirs(model_save_dir, exist_ok=True)  # Ensure directory exists



    results={}
    curve_save_dir = "/root/code/thesis/codeFolder/LatestDataInUse/results/curves/Resnet/MRICombineChannelWise/Image"
    ...
    for lr in learning_rates:
        ...
        survival_results_df.to_csv(f'/root/code/thesis/codeFolder/LatestDataInUse/results/survival/ResNet50/MRICombineChannelWise/Image/survival_results_lr_{best_learning_rate}.csv', index=False)
        # Save classification report to file
        with open(f'/root/code/thesis/codeFolder/LatestDataInUse/results/reports/Resnet/MRICombineChannelWise/Image/classification_report_lr_{best_learning_rate}.txt', 'w') as f:
    ...
    result_file = "/root/code/thesis/codeFolder/LatestDataInUse/results/MRT3Sequences_Channelwise_ImageResNet.txt"
