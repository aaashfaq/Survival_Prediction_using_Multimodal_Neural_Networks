...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_path_e_thrive = self.df.loc[index, 'e_thrive']
        image_path_s3t1_wats_neu_Augmented = self.df.loc[index, 's3t1_wats_neu_Augmented']
        image_path_t2_hr_spir_range_Augmented = self.df.loc[index, 't2_hr_spir_range_Augmented']




        img_e_thrive = Image.open(image_path_e_thrive).convert("L")
        img_e_thrive= np.array(img_e_thrive)

        img_s3t1_wats_neu_Augmented = Image.open(image_path_s3t1_wats_neu_Augmented).convert("L")
        img_s3t1_wats_neu_Augmented= np.array(img_s3t1_wats_neu_Augmented)

        img_t2_hr_spir_range_Augmented = Image.open(image_path_t2_hr_spir_range_Augmented).convert("L")
        img_t2_hr_spir_range_Augmented= np.array(img_t2_hr_spir_range_Augmented)
        ...
        if self.transform is not None:
            img_e_thrive = self.transform(img_e_thrive)
            img_s3t1_wats_neu_Augmented=self.transform(img_s3t1_wats_neu_Augmented)
            img_t2_hr_spir_range_Augmented=self.transform(img_t2_hr_spir_range_Augmented)


        return img_e_thrive,img_s3t1_wats_neu_Augmented, img_t2_hr_spir_range_Augmented, tabular, label
...
class MyDatasetTest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_e_thrive,img_s3t1_wats_neu_Augmented,img_t2_hr_spir_range_Augmented, tabular, label, patientsID
...
class CustomModel(nn.Module):
    ...
    def forward(self, img1, img2, img3, clinical_features):
        # Concatenate images channel-wise
        concatenated_image= torch.cat((img1, img2, img3), dim=1)
        features = self.resnet50(concatenated_image)

        clinical_features = torch.squeeze(clinical_features, dim=1)

        extended_features = self.embedding(clinical_features)

        # Concatenate the ResNet-50 output and extended features
        x = torch.cat([features, extended_features], dim=-1)
        #print('shape', x.shape)
...
def train_epoch(net, trainloader, criterion, optimizer):
    ...
    for i, data in enumerate(trainloader):
        img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular, labels = data

        img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular, labels = img_e_thrive.to(device), img_s3t1_wats_neu.to(device), img_t2_hr_spir_range.to(device),tabular.to(device), labels.to(device)

        optimizer.zero_grad()
        outputs = net(img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular)
...
def valid_epoch(net, valid_loader, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(valid_loader, 0):
            ...
            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular, val_labels , patientsID = data

            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular, val_labels = img_e_thrive.to(device), img_s3t1_wats_neu.to(device), img_t2_hr_spir_range.to(device), tabular.to(device), val_labels.to(device)
...
def test_epoch(net, valid_loader, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(valid_loader, 0):
            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular, test_labels, patientsID = data

            img_e_thrive, img_s3t1_wats_neu, img_t2_hr_spir_range, tabular, test_labels = img_e_thrive.to(device), img_s3t1_wats_neu.to(device), img_t2_hr_spir_range.to(device), tabular.to(device), test_labels.to(device)
...
if __name__ == '__main__':
    ...
    model_save_dir = "/root/code/thesis/codeFolder/LatestDataInUse/results/models/Resnet/MRI4Sequences/Multimodal"
    os.makedirs(model_save_dir, exist_ok=True)  # Ensure directory exists



    results={}
    curve_save_dir = "/root/code/thesis/codeFolder/LatestDataInUse/results/curves/Resnet/MRICombineChannelWise/Multimodal"
    ...
    for lr in learning_rates:
        ...
        for train_index, test_index in stratified_group_kfold.split(X, y, groups=dataset['PatientID']):
            ...
            train_loader = DataLoader(train_dataset, batch_size=16, sampler= training_sampler, drop_last=True)
            test_loader = DataLoader(test_dataset, batch_size=16, sampler= validation_sampler, drop_last=True)
        ...
        survival_results_df.to_csv(f'/root/code/thesis/codeFolder/LatestDataInUse/results/survival/ResNet50/MRICombineChannelWise/Multimodal/survival_results_lr_{best_learning_rate}.csv', index=False)
        # Save classification report to file
        with open(f'/root/code/thesis/codeFolder/LatestDataInUse/results/reports/Resnet/MRICombineChannelWise/Multimodal/classification_report_lr_{best_learning_rate}.txt', 'w') as f:
    ...
    result_file = "/root/code/thesis/codeFolder/LatestDataInUse/results/MRIChannelwise_MultimodelResNet50.txt"
